...
from PyQt5.QtWidgets import (QWidget, QVBoxLayout, QHBoxLayout, QLabel, QLineEdit, QPushButton, QComboBox,
                             QScrollArea, QTableWidget, QTableWidgetItem, QHeaderView, QAbstractItemView, QGroupBox,
...
class PublisherMessageViewer(QWidget):
    ...
    def showDetails(self, item):
        ...
        if row < len(self.pubMessages):
            # Se utiliza JsonDetailDialog de common/utils.py o definido en este módulo
            from common.utils import JsonDetailDialog  # Asegúrate de que esta importación es correcta
            dlg = JsonDetailDialog(self.pubMessages[row], self)
            dlg.exec_()


# --- Clases de la interfaz de Publicador ---
# Aquí se incluye también el código para el resumen de mensajes y la parte de edición.
# Debido a la extensión del código, se pueden separar en funciones auxiliares o en otros ficheros.
# A modo de ejemplo, incluyo la clase MessageConfigWidget y PublisherTab:
...
class MessageConfigWidget(QGroupBox):
    ...
    def initUI(self):
        ...
        # Se puede reutilizar aquí un editor (por ejemplo, PublisherEditorWidget)
        
        from .pubEditor import PublisherEditorWidget  # Suponiendo que se separe el editor en otro fichero
...
# (Aquí se incluirían otras clases del publicador, como el editor, si se separa en otros ficheros)

# Para simplificar, se muestra la integración de PublisherTab y MessageConfigWidget en este módulo.
